...
-How is the velocity of the ball determined?
	The velocity of the ball once launched is determined using the rand() function and the current level of the game. The velocity is split into x and y components, that are floats that have a value between level and level+1. This ensures that the game becomes more difficult if the level progresses.  The velocity is also re-randomized everytime it hits the paddle.
